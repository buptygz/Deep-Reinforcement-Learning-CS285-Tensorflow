...
class FFModel(BaseModel):
    ...
    #############################
    def extract_statistics(self, data_statistics):
        return {
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        modif_stats = self.extract_statistics(data_statistics)
        modif_stats.update({self.obs_pl: observations, self.acs_pl: actions})
        return self.sess.run(self.next_obs_pred, feed_dict=modif_stats) # TODO(Q1) Run model prediction on the given batch of data

    def update(self, observations, actions, next_observations, data_statistics):
        # train the model

        modif_stats = self.extract_statistics(data_statistics)
        modif_stats.update({self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: (next_observations - observations)})
        _, loss =  self.sess.run([self.train_op, self.loss], feed_dict=modif_stats) # TODO(Q1) Run the defined train_op here, and also return the loss being optimized (on this batch of data)
